...
func Configure(m *wserver.Manager, client remote.Client) *gin.Engine {
	...
	protected.POST("/api/soar/databases/:engine/databases", postDatabase)
	protected.DELETE("/api/soar/databases/:engine/databases/:database", deleteDatabase)
	protected.PATCH("/api/soar/databases/:engine/databases/:database/password", patchDatabasePassword)
	protected.DELETE("/api/soar/databases/:engine/engine", deleteDatabaseEngine)
	...
	server.Use(middleware.RequireAuthorization(), middleware.ServerExists())
	{
		...
		server.GET("/l7/status", getServerL7Status)
		server.POST("/l7/enable", postServerL7Enable)
		server.POST("/l7/disable", postServerL7Disable)
